...
class Discriminator(nn.Module):
    def __init__(self, nc=3, ndf=64, use_spectral_norm=False, use_sigmoid=True):
...
def test_discriminator():
    ...
    print("Testing Discriminator without spectral normalization (BCE mode)...")
    netD = Discriminator(nc=3, ndf=64, use_spectral_norm=False, use_sigmoid=True).to(device)
    ...
    print("\nTesting Discriminator with spectral normalization (LSGAN mode)...")
    netD_sn = Discriminator(nc=3, ndf=64, use_spectral_norm=True, use_sigmoid=False).to(device)
    
    with torch.no_grad():
        output_sn = netD_sn(real_images)
    
    print(f"Discriminator (SN/LSGAN) output shape: {output_sn.shape}")
    print(f"Min value: {output_sn.min().item():.3f}, Max value: {output_sn.max().item():.3f}")
